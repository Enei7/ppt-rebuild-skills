#!/usr/bin/env python3
"""Bundled editable-slide CLI for the PPT Rebuild skill."""
...
def cmd_next(args: argparse.Namespace) -> int:
    ...
    local_busy = args.local and any(page.get("status") == "dispatched" for page in pages)
    if dispatchable and slots > 0 and not local_busy:
        selected = dispatchable[:1] if args.local else dispatchable[:slots]
        first_page = find_page(jobs, selected[0])
        prompt_out = page_dir_for(run_dir, first_page) / "worker-prompt.md"
        if args.local or (len(pages) == 1 and selected == [first_page.get("page_id")]):
            payload = {
                "run_dir": str(run_dir),
    ...
    if unfinished:
        payload = {
            ...
            "agent_focus": (
                "Complete and record the currently claimed local page before requesting the next page."
                if args.local else
                "Wait for dispatched workers, then record completed page results. Do not reset slow active workers; use `run reset` only after failure, terminal state, cancellation, or lost-worker verification."
            ),
        }
        return print_json(payload) if args.json else _print_next_text(payload)
...
""",
  ...
  editppt prepare a.png b.png --out-root output/ppt-rebuild
""",
    )
...
""",
    ...
    run_next.add_argument("--local", action="store_true", help="Rebuild pending pages sequentially in the main agent when page workers are unavailable.")
...
""",
    ...
    dispatch = run_sub.add_parser(
        ...
        description="Mark a page as dispatched after a worker/thread has been created, or after the main agent claims one page with --local.",
        formatter_class=HELP_FORMATTER,
    )
    ...
    dispatch.add_argument("--local", action="store_true", help="Claim one page for main-agent reconstruction; multi-page runs may proceed sequentially.")
    ...
    formula = sub.add_parser(
        ...
        description="""Render temporary LaTeX image assets for page manifests.

The final PPT Rebuild deck must replace required formula images with native
Office Math using scripts/equationize.py after deck finalization.
""",
        formatter_class=HELP_FORMATTER,
    )
...
This is a temporary page-stage formula path. It relies on a local TeX engine
(xelatex, lualatex, or pdflatex). SVG output additionally requires dvisvgm or
pdf2svg; PNG output requires ImageMagick. The formula image must be replaced
with a native editable equation before delivery.
""",
        formatter_class=HELP_FORMATTER,
        epilog="""Examples:
