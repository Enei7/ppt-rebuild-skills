...
`image_backend` is written by `editppt prepare` and may be overwritten by `editppt run backend` when needed. Parent-level backend selection policy lives in `SKILL.md` under "Page reconstruction and state".
...
Other backend metadata may describe model labels, runtime homes, or handoff text, but it does not change this order. Parent-level tool selection and user-interaction policy live in `SKILL.md` under "Page reconstruction and state"; page reconstructors execute the copied contract above.
...
`dispatch` is written by `editppt run dispatch`. It includes `execution_mode`: `"worker"` for normal page-worker dispatch and `"local"` for the parent agent's one-at-a-time local claim (also valid in a multi-page run); older dispatch records without this field are treated as `"worker"`. A page with status `dispatched` is an active execution lease until explicit completion, failure, cancellation, or lost-worker verification; elapsed time alone does not make it lost. `result` is written by `editppt run record`. `accepted` is written by `editppt run finalize`.
...
New `visual_inventory` entries should declare `role` (`foreground`, `background`, `structure`, or `formula`) and may supply `object_type` (such as `icon` or `photo`). For `role: foreground`, supply `path` matching an image and its `asset_provenance`, plus `source_type` matching that provenance. The normal sources are `asset-sheet-separated` or `imagegen`. When the user requests an exact PDF figure, logo, or QR-code crop, `user-provided` is allowed only with `exact_source_crop: true`, a one-based `source_page`, a source-pixel `source_crop_px: [x, y, width, height]`, and an existing `source` file. Keep the crop as a selectable bitmap; do not call it vector-editable. Descriptions and provenance notes explain the work; they are not keyword-based proof of the source method. Legacy entries remain supported; prefer these fields for unambiguous classification.
  ...
  "asset_provenance": [
    {
      ...
      "provenance_note": "Temporary page-stage formula image; the final deck must replace it with native Office Math."
    }
  ],
  "formula_inventory": [
    {
...
State and provenance record rules are described in `SKILL.md` under "Page reconstruction and state" and "PDF source fidelity", and in the asset processing examples in `cli-helper.md`.
