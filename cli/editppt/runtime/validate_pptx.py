...
def foreground_asset_contract_violations(manifest):
    violations = []
    allowed_foreground_sources = {"asset-sheet-separated", "imagegen", "user-provided"}
    provenance_by_path = {
    ...
    for index, item in enumerate(manifest.get("visual_inventory", [])):
        ...
        if (not structured and has_forbidden_decision(decision)) or (declared_source is not None and declared_source not in allowed_foreground_sources):
            violations.append({"field": field, "reason": "foreground visual decisions require an approved source: separated/generated asset or documented exact PDF crop"})
        ...
        if path:
            ...
            if not provenance or provenance.get("source_type") not in allowed_foreground_sources:
                violations.append({"field": field, "path": path, "reason": "foreground visual objects require matching asset provenance"})
    ...
    for index, entry in enumerate(manifest.get("asset_provenance", [])):
        ...
        is_foreground = path in foreground_paths or is_foreground_visual_item(entry)
        if is_foreground and entry.get("source_type") not in allowed_foreground_sources:
            violations.append({"field": f"asset_provenance[{index}]", "path": path, "reason": "foreground asset provenance must use asset-sheet-separated, imagegen, or documented user-provided crop"})
        if is_foreground and entry.get("source_type") == "user-provided":
            crop = entry.get("source_crop_px")
            page = entry.get("source_page")
            documented = (
                entry.get("exact_source_crop") is True
                and isinstance(page, int) and not isinstance(page, bool) and page > 0
                and isinstance(crop, (list, tuple)) and len(crop) == 4
            ...
            if not documented:
                violations.append({"field": f"asset_provenance[{index}]", "path": path, "reason": "user-provided foreground requires exact_source_crop, source_page, and source_crop_px"})
    return violations
