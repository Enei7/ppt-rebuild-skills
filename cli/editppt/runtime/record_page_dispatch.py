...
def main():
    ...
    parser.add_argument("--local", action="store_true", help="Claim one page for main-agent local reconstruction.")
    ...
    if args.local and any(other.get("status") == "dispatched" for other in jobs.get("pages", [])):
        raise SystemExit("--local dispatch requires all other pages to be recorded or pending")
