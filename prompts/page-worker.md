...
8. Never put a multiline `\substack` text annotation under an integral or summation inside one formula. It can generate Office Math that PowerPoint refuses to open. Build the native operator/formula separately from editable annotation text/math, preserving their source positions; open-test the converted page.
