...
For each standalone mathematical expression, create a unique per-page id and a source-pixel placement box. Preserve exact variables, indices, matrices, delimiters, and display alignment in its LaTeX source. A sample inventory entry follows; adapt its `image` path to the actual intermediate formula object.

`formula_inventory[].box_px` is the target placement for the temporary formula and later native equation. When the temporary image is an exact crop whose original source rectangle differs from that placement, `asset_provenance` and `visual_inventory` must carry the matching `source_crop_px` and `source_page`. Source-geometry tools join those records through the formula image/id: they measure glyph ink and height in the true source crop, then translate the expected ink center to the target placement without scaling it to the target box. Conflicting crop records, a mismatched source page, or incomplete exact-crop metadata are errors. A legacy formula with no exact-crop metadata uses `box_px` for both roles.
...
Overlines must remain above their operands. The Office stylesheet maps some `\overline` expressions to a horizontal-bar accent whose glyph crosses the letter (for example a barred V can look like a different symbol). The converter normalizes this case to a structural native top bar. Check accent placement in the PowerPoint render as well as the LaTeX; correct source transcription alone does not guarantee correct visual meaning.
...
Do not use `\tfrac` inside matrix/determinant environments with the current converter: its compact-fraction wrapper can produce a package that passes XML checks but PowerPoint refuses to open. The converter rejects this known combination. Use ordinary `\frac` inside the matrix. When a determinant and its neighboring fraction have different source text levels, inventory those source blocks separately so fitting the tall matrix does not shrink the entire expression. Always test the resulting matrix page in PowerPoint before converting the whole deck.
...
For dark-on-light formulas on a uniform background, `refine_math_geometry.ps1` implements this targeted pass. Supply a JSON array such as `[{"page":7,"id":"formula_02"}]` whose one-based pages and IDs match the run inventory, then run `powershell -NoProfile -ExecutionPolicy Bypass -File <skill-root>/scripts/refine_math_geometry.ps1 -Deck <candidate.pptx> -Run <run> -Targets <targets.json> -Output <new-output.pptx> -Python <same-python>`. It exports isolated ink before scaling, after scaling, and after alignment, preserving evidence in a unique run subfolder. Review the source crop first: neighboring prose or dark background can invalidate its measurement. Scale and shift limits fail rather than silently applying implausible adjustments. Use `-AlignOnly` when per-run glyph sizes must stay unchanged. `-FirstPage N` is only for a test/subdeck whose first slide corresponds to source page N; omit it for a complete deck. Rerender the resulting full slides, rerun collision/inventory checks, and inspect them before delivery; a good ink-bound match is not semantic or visual proof.

Any manual native-math font-size change invalidates ink `dx`/`dy` measured before that edit. Save the resized deck, rerender and remeasure its current ink, then run `-AlignOnly` or mixed-line reflow from the fresh measurement; never reuse pre-resize translations. `refine_math_geometry.ps1` already rerenders and remeasures after resizing and before alignment; preserve that measurement order in manual repair workflows.

When a source-verified black or gray formula is unavoidably overlapped by a colored annotation, a refinement target may opt in with `{"page":17,"id":"formula_02","source_ink_filter":"achromatic"}`. The isolated source measurement then ignores pixels whose RGB channel range exceeds 60; rendered-equation measurement is unchanged. This option belongs only in the refinement targets JSON, not a page manifest. Use it only after verifying that the formula glyphs themselves are achromatic and the rejected color belongs to an annotation. Do not enable it for colored or mixed-color formulas. An unknown filter value, or a crop with no measurable source ink after filtering, is an error rather than a fallback to unfiltered geometry.
