...
def build_prompt(run_dir: Path, page: dict, page_dir: Path) -> str:
    ...
    replacements = {
        ...
        "{{EDITPPT_CMD}}": f'"{sys.executable}" "{SKILL_ROOT / "scripts" / "editppt.py"}"',
    }
...
def main() -> int:
    ...
    parser.add_argument("--cli", default=f'"{sys.executable}" "{SKILL_ROOT / "scripts" / "editppt.py"}"', help="CLI command used in the returned dispatch command template.")
