...
- Transcribe LaTeX from the visible source; OCR hints do not prove symbols, indices, matrices, or delimiters. Every standalone formula, and every inline formula the user requires in the equation editor, needs a unique `formula_inventory` id, accurate placement `box_px`, and `latex` or `tex_source`. Reference the temporary image with `image`/`replace_image_id` so conversion replaces it rather than overlaying it. When an exact temporary source crop is moved or its placement padding differs, keep the true crop in matching `asset_provenance` / `visual_inventory` `source_crop_px`; calibration resolves that source evidence separately from placement instead of reading source ink from `box_px`.
- Before native conversion/calibration, run `python <skill-root>/scripts/review_formula_crops.py --run <run> --output <new-review-folder>` (or add `--page N` while authoring). Inspect the source-crop boards and full-size crops: include every limit, derivative accent, trailing condition, and closing delimiter; exclude neighboring lines and decorative borders. Repair incorrect boxes through the page-owner workflow. Edge flags are review candidates, not proof of clipping; a structurally valid manifest with a clipped source crop is not a calibration reference.
...
- Check tall delimiters in the actual PowerPoint render: MathML-to-Office conversion may shrink `\left\{...\right\}` even when the LaTeX and temporary crop are correct. If the native fence remains too small, split only the braces into editable PowerPoint vector shapes sized to the source and keep the enclosed math as native Office equations. Do not cover the equation with a bitmap; document that the fence and math are separate editable objects.
...
- When calibrated formulas still collide with adjacent prose, repair the whole source sentence with an explicit [mixed-line reflow recipe](references/reflow-mixed-lines.md). Measure actual rendered widths, preserve formula sizes, attach punctuation without a word-sized gap, and fail on overflow rather than silently shrinking the line.
...
Check prose completeness separately from formula counts: every visible heading, sentence, condition, conjunction and annotation must be accounted for. A formula-only reconstruction of a prose-rich page is a failure even when every inventoried equation converts. Verify negative signs, equality/inequality symbols, powers, primes and piecewise braces against the source; do not infer acceptance from a plausible mathematical result.
...
- `scripts/review_formula_crops.py`: export formula-only source evidence and suspicious-edge candidates before native conversion. Inspect boards plus full-size crops where necessary; this helper does not certify mathematical completeness.
- `scripts/reflow_mixed_lines.ps1`, `scripts/preflight_reflow_mixed_lines.py`: apply explicit source-reviewed prose/math line recipes without resizing formulas or silently fitting overflow; see [references/reflow-mixed-lines.md](references/reflow-mixed-lines.md).
